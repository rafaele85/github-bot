...
Если Chat сервис недоступен или вернул ошибку: Будет достаточно просто игнорить это событие. 
...
6. ChatController подготавливает по схеме GraphQL мутацию NewMessages2 , в которую добавляет строчку сообщения и 
   выполняет GraphQL запрос на API Chat.  
...
Для модульного тестирования реализованы mock тестовые классы для ChatController, GithubController.
Тестирование производится путем имитации сообщения от github и передачи его сразу на вход TestGithubController, который
в свою очередь передает его в сервис. Прогоняется вся цепочка Github - событие - подписчики - ChatService - TestChatController.
TestChatController подготавливает graphQl запрос, но не отсылает его, а все его параметры сохраняет в  TestStorage.
...
* TODO Можно реализовать обработку других типов событий от github, поисследовать использование API Github Actions
  вместо Webhooks 
* TODO ChatController: реализовать надежную доставку сообщений (с конфигурированием количества попыток и интервала между ними)
